...
def market_open(context):
    ...
    if len(context.portfolio.positions) > 0:        
        ...
        for stock in securities:
            ...
            if hold_cost/price[stock]-1 < -0.01:
                order_target(stock, 0)
                log.info('%s 止损：%s' %(last_date,stock))  
            #if price[stock]/hold_cost > 1.2:
            #    order_target(stock, 0)
            #    log.info('%s 获利了结：%s' %(last_date,stock))                  
            ...
            #print(df['hist'][0])
            #print(df['hist'][1])
            #if df['DIFF'][0]<= df['DEA'][0] and df['DIFF'][1]>df['DEA'][1]:
            #    order_target(stock, 0)
            if df['DIFF'][0]>0 and df['DEA'][0]>0 and df['hist'][0]>0 and df['hist'][0]< df['hist'][1] :#and df['hist'][1]<df['hist'][2]:
                # 全部卖出
                order_target(stock, 0)
                # 记录这次卖出
    ...
    stoploss1 = dp_stoploss(kernel=1, n=89, zs=0.07)
    stoploss2 = dp_stoploss(kernel=2, n=3, zs=0.05)
    if stoploss2 or stoploss1 :  # or  g.days % g.period == 0:
        if len(context.portfolio.positions)>0:
            for stock in list(context.portfolio.positions.keys()):
                order_target(stock, 0)
            g.days = 0

    if  (not stoploss2) or (not stoploss1):
        if len(stock_list) > 0:
            for stock in stock_list:
                if stock not in list(context.portfolio.positions):
                    if len(context.portfolio.positions) < g.stocknum :      
                        ...
                        #print(df['hist'][0])
                        #print(df['hist'][1])
                        #if df['DIFF'][0]<df['DEA'][0] and df['DIFF'][0]<0 and df['DEA'][0] <0 and df['hist'][0] <0  and  df['hist'][1] <df['hist'][0] and df['hist'][2] < df['hist'][1] :
