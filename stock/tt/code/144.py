...
def before_market_open(context):
    ...
    if len(list_stocks) > 0:        
        securities = list_stocks
        for stock in securities:
            df= history(50,unit='1d',field='close', security_list=stock, df=True,skip_paused=True,fq='pre')
            print(df)
            #dfd['MACD'],dfd['MACDsignal'],dfd['MACDhist'] = ta.MACD(df['close'].values,fastperiod=13, slowperiod=21, signalperiod=8)
            #s1 = (dfd['MACD']>0) #macd 黑线 快线
            #s2 = (dfd['MACDsignal']<0)    #signal 红线 慢线  
            #s31 = (dfd['MACDhist']>0)  #hist红柱    
            #s32 = (dfd['MACDhist']<0)  #hist绿柱                
            #s4 = (dfd['MACDhist']<dfd['MACDhist'].shift(-1))   
            #s5 = (dfd['MACD']>dfd['MACDsignal']            
            #condition1 = (dfd['MACDhist']>0)
            # 当天的短期均线大于等于长期均线   
            #condition2 = (dfd['MACD'] >= dfd['MACDsignal'])
            # 上个交易日的短期均线小于长期均线        
            #condition3 = (dfd['MACD'].shift(-1) < dfd['MACDsignal'].shift(-1))   
            # 将买入信号当天的signal设置为1   
            #df.loc[condition1 & condition2 & condition3, 'signal'] = '金叉'         
            #===找出卖出信号    
            # 当天的短期均线小于等于长期均线
            #condition1 = (dfd['MACD'] <= dfd['MACDsignal'])
            # 上个交易日的短期均线大于长期均线
            #condition2 = (dfd['MACD'].shift(-1) > dfd['MACDsignal'].shift(-1))
            # 将买入信号当天的signal设置为-1
            #if condition1 and condition2 ：
            #    order_target(stock, 0)
            #if s1 and s31 and s4:
            #    order_target(stock, 0)
