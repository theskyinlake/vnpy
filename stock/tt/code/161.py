...
def before_market_open(context):
    ...
    if len(list_stocks) > 0:        
        ...
        for stock in securities:
            ...
            s5 = (df['DIFF']>df['DEA']            
            condition1 = (df['hist']>0)
            # 当天的短期均线大于等于长期均线   
            condition2 = (df['DIFF'] >= df['DEA'])
            # 上个交易日的短期均线小于长期均线        
            condition3 = (df['DIFF'].shift(-1) < df['DEA'].shift(-1))   
            # 将买入信号当天的signal设置为1   
            df.loc[ s1 & condition1 & condition2 , 'signal'] = '买入'         
            
            #===找出卖出信号    
            # 当天的短期均线小于等于长期均线
            #condition1 = (dfd['DIFF'] <= dfd['DEA'])
            # 上个交易日的短期均线大于长期均线
            #condition2 = (dfd['DIFF'].shift(-1) > dfd['DEA'].shift(-1))
            # 将买入信号当天的signal设置为-1
